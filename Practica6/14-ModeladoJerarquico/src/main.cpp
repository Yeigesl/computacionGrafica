...
Shader shader;
//crear objeto tipo camara
std::shared_ptr<FirstPersonCamera>camera(new FirstPersonCamera());
//equivalente solo debe lberarse paa la sigiente la camara
//FirstPersonCamera*camera=new
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	//sensibilidad de la camara , la cual por default esta  a 25.0 f 
...
bool processInput(bool continueApplication) {
	...
	//suaviza el movimiento para que se mueva de acuerdo al renderizadfo
	deltaTime = TimeManager::Instance().DeltaTime;
	//Cachamos eventos cuando se presiona una tecla 
...
void applicationLoop() {
	...
	while (psi) {
		...
		//codo derecho 
		//codo der
		matrixs7 = glm::scale(matrixs7, glm::vec3(1.0f, 1.0f, 1.0f));
		sphere.setProjectionMatrix(projection);
		sphere.setViewMatrix(view);
		sphere.enableWireMode();
